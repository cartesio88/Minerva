...
MLBActuatorVisibility::MLBActuatorVisibility(const std::string& name, MAO& parent, bool value): MLBActuator(name, parent) {
	_value = value;
	_mlbType = T_MLBACTUATORVISIBILITY;
	if(parent.getType()!=T_MAORENDERABLE2DTEXT && parent.getType()!=T_MAORENDERABLE3DORJ &&
	   parent.getType()!=T_MAORENDERABLE2DIMAGE && parent.getType()!=T_MAORENDERABLE3DTEAPOT &&
	   parent.getType()!=T_MAORENDERABLE3DLINE && parent.getType()!=T_MAORENDERABLE3DPATH){
	  Logger::getInstance()->error("MAO type is not applicable to MLBActuatorVisibility!: "+parent.getName());
	  throw "MAO type is not applicable to MLBActuatorVisibility!: "+parent.getName();
	}
}
